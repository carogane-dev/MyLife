...
- **CORS** (`api/src/index.ts`) : `CORS_ORIGIN` accepte une liste séparée par des virgules. `tauri dev` fonctionne tel quel (la fenêtre charge `http://localhost:5173`, déjà autorisé). **`tauri build` n'a jamais été testé** : le front packagé sera servi depuis un protocole interne à Tauri (pas `http://localhost:5173`) — il faudra ajouter cette origine à `CORS_ORIGIN` dans `api/.env` le jour où on teste vraiment un build.
...
- **Pas encore vérifié** : ni `tauri dev` ni `tauri build` n'ont pu être testés dans cette session (Build Tools pas encore installés côté utilisateur au moment de l'implémentation) — à faire dès qu'ils le sont, avant de merger la branche `feature/packaging-tauri-desktop`.
